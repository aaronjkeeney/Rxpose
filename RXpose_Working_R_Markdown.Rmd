...
SELECT weight_class_kg, athlete_full_name, sinclair
FROM weightlifting_results
ORDER BY sinclair DESC
LIMIT 30
...
```{r}
# Force encoding in basic Latin English alphabet
...
# Removal of all spaces, non-letter characters, and spaces. This can probably be optimized with a few pipes.
...
```{r}
positive_testing_lifters_closest_match <- stringdist_inner_join(positive_testing_lifters_without_exact_matches, weightlifting_results_without_exact_matches, by = "athlete_full_name", method = "cosine", max_dist = 0.08)
view(positive_testing_lifters_closest_match)
...
```
Cosine similarity and Jaccard distance give slightly different results, but with the approximate matching, we are able to add between 10 and 30 other names.  This approximate matching has done a good job of catching missing characters or middle names. It also matches Asian names that are sometimes transcribed into English in differing orders. Finding the correct cutoff for the maximum distance will take some trial and error. Major considerations are:

* Misspellings and omitted characters need to be matched.

* Missing or reordered names need to be matched. Fortunately, cosine similarity is perfect for this.

* Many names are similar, and multiple generations of the same family often participate in the same sport. Parents and children should not match. Unfortunately, cosine similarity makes this tough.



It would be good to check to see if the original dataset includes some athletes who had medals stripped (Klokov is a famous example). Also recent positive tests may not be included (Lu Xiaojun)
```{sql connection=con}
